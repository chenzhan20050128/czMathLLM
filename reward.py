...
def score_math_answer(prediction: str, reference: str, *, metadata: dict | None = None) -> float:
    ...
    # Encourage longer, coherent reasoning when difficulty is high
...
def batch_reward(predictions: Iterable[str], references: Iterable[str], metadatas: Iterable[dict]) -> list[float]:
    return [score_math_answer(pred, ref, metadata=meta) for pred, ref, meta in zip(predictions, references, metadatas)]
